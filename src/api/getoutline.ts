...
const setOptions = (bodyOptions = {}) => {
	return {
		...
		headers: {
			...
			Authorization:
				'Bearer ol_api_Hx89vIcTFOFdNDpAeaqHA19qzoqLbjieutvHwD'
		},
...
export const fetchAllCollections = async (bodyOptions?: string) => {
	...
	try {
		...
		const data: CollectionsData[] = await response
			.json()
			.then((body) => body.data);
		return data;
	} catch (error) {
...
export const fetchCollectionsDocuments = async (bodyOptions?: string) => {
	const collectionsPath = 'collections.documents';
...
export const fetchAllDocuments = async (bodyOptions?: object) => {
	...
	try {
		...
		const data: DocumentsData[] = await response
			.json()
			.then((body) => body.data);
		return data;
	} catch (error) {
